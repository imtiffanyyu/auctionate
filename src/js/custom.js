$(document).ready(function(){
	...
	$(".editbidder").click(function() {
		$("#editbidderform").fadeIn('slow');

			var clickedbidder = $(".clickedbidderid", this).text() // looking for clickedbidderid in the context of this (the thing I clicked)
			console.log("You clicked bidder " + clickedbidder)

			$.ajax({
				method: "GET",
				url: "/bidderjson",
				data: {
					clickedbidder: clickedbidder
				}
			}).done(function(bidder){
				console.log(bidder)
				$('#displaybidderid').text (bidder.id)
				$('#firstname').text (bidder.firstname)
				$('#lastname').text (bidder.lastname)
				$('#address').text (bidder.address)
				$('#zipcode').text (bidder.zipcode)
				$('#city').text (bidder.city)
				$('#country').text (bidder.country)
				$('#phone').text (bidder.phone)
				$('#email').text (bidder.email)
				$('#payment').text (bidder.payment)
				$('#shipping').text (bidder.shipping)
			})


		})

	$(".deletebidder").click(function() {
		var deletebidderid = $("#displaybidderid").text();
		console.log(deletebidderid);

		$.ajax({
			method: "DELETE",
			url: "/bidder",
			data: {
				deletebidderid: deletebidderid
			}
		})
	})

	$(".beditlink").on("click", function(e){
		e.preventDefault();
		var dataset = $(this).prev(".datainfo");
		var savebtn = $(this).next(".bsavebtn");
		var theid   = dataset.attr("id");
		var newid   = theid+"-form";
		var currval = dataset.text();
	...
	$(".bsavebtn").on("click", function(e){
		e.preventDefault();

		var displaybidderid = $("#displaybidderid").text();
		console.log(displaybidderid);

			var elink   = $(this).prev(".beditlink"); // looks for "editlink" before savebtn
			var dataset = elink.prev(".datainfo"); // looks for "datainfo" before editlink
			var newid   = dataset.attr("id"); // grabs id for form creaton
			var cinput  = "#"+newid+"-form"; // creates form for the id
			var einput  = $(cinput);
			...
			$.ajax({
				...
				data: {
					newid: newid,
					newValue: newval,
					displaybidderid: displaybidderid
				}
			})
		});
});